...
async def _call_dify_workflow_anchor(keyword: str, user_id: str) -> str:
    ...
    # 兼容另一套命名（避免你别的文件用 DIFY_API_KEY 时这里拿不到）
    if not api_key:
        api_key = os.getenv("DIFY_API_KEY", "").strip()

    # 分段 timeout：连接短，读取长
    connect_s = float(os.getenv("ANCHOR_CONNECT_TIMEOUT_SECS", "5"))
    read_s = float(os.getenv("ANCHOR_READ_TIMEOUT_SECS", "120"))
    write_s = float(os.getenv("ANCHOR_WRITE_TIMEOUT_SECS", "30"))
    pool_s = float(os.getenv("ANCHOR_POOL_TIMEOUT_SECS", "5"))
    ...
    for attempt in range(3):
        try:
            ...
            print(f"[anchor] dify_timeout attempt={attempt+1}/3 err={type(e).__name__}: {e}")
            if attempt < 2:
                # 简单退避：0.5s, 1.5s
                import asyncio
...
async def chat_completions(request: Request):
    ...
    # ✅ 关键：如果本次请求带 tools/functions，说明要让上游“看见工具调用+工具结果”
    # 这时绝不能删 role=tool / tool_calls，否则会上游无限循环调用工具。
    has_tools = bool(payload.get("tools") or payload.get("functions"))
    if not has_tools:
    # 没带 tools 的普通聊天才需要清理历史里的工具痕迹，防止上游 400
