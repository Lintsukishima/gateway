...
def run_s4(db: OrmSession, session_id: str, to_turn: int, window: int = 16, model_name: str = "unknown"):
    exists = (db.query(SummaryS4)
          .filter(SummaryS4.session_id == session_id, SummaryS4.to_turn == to_turn)
          .first())
    if exists:
           return {"skipped": True, "reason": "s4 already exists", "to_turn": to_turn}
...
def run_s60(db: OrmSession, session_id: str, to_turn: int, window: int = 60, model_name: str = "unknown"):
    exists = (db.query(SummaryS60)
          .filter(SummaryS60.session_id == session_id, SummaryS60.to_turn == to_turn)
          .first())
    if exists:
           return {"skipped": True, "reason": "s60 already exists", "to_turn": to_turn}
